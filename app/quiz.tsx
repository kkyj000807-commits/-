...
export default function QuizScreen() {
  ...
  const playSound = useCallback((type: "correct" | "wrong") => {
    ...
    try {
      if (type === "correct") { correctPlayer.seekTo(0); correctPlayer.play(); }
      else { wrongPlayer.seekTo(0); wrongPlayer.play(); }
    } catch {}
  ...
  const handleChoiceSelect = useCallback(
    ...
    [answered, q, haptic, animateCard, playSound]
  );
  ...
  const handleFlashGrade = useCallback(
    (grade: "correct" | "wrong") => {
      ...
      playSound(grade === "correct" ? "correct" : "wrong");
      setFlashGrade(grade);
      setAnswered(true);
      if (grade === "correct") {
        setCorrectCount((c) => c + 1);
  ...
  const handleTypeSubmit = useCallback(() => {
    ...
    // 타이핑 정답 즉시 저장
    recordOneAnswer(isCorrect, isCorrect ? undefined : q.item.num);
  }, [typedAnswer, q, haptic, playSound]);
      ...
      >
        ...
        >
          ...
          <View style={s.topBar}>
            ...
            <Text style={s.topBarTitle}>{getModeLabel()}</Text>
            <View style={{ width: 72 }} />
          </View>
                ...
                <FlipCard
                  ...
                  back={
                    <View style={s.flipBack}>
                      {/* 다시 가리기 버튼 */}
                      {!answered && (
                        <Pressable style={s.hideAgainBtn} onPress={handleReveal}>
                          <Text style={s.hideAgainBtnText}>↩ 다시 가리기</Text>
                        </Pressable>
                      )}
